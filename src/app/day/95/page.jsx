import './page.css';

import NivoBar from '@internal/components/Charts/Nivo/Bar';
import NivoChoropleth from '@internal/components/Charts/Nivo/Choropleth/Choropleth';

import { infos } from './config';
import { avgRegion, mapData } from './utils/getParsedData';

const Page95 = () => {
  return (
    <>
      <section id='infos'>
        <h1 className='py-2 font-mono text-4xl font-bold page-title'>
          {infos.title}
        </h1>
        <p className='py-2 font-mono text-lg'>{infos.description}</p>
      </section>
      <section id='map' className='mt-10'>
        <h3 className='py-2 font-mono text-3xl max-md:text-2xl text-center flex max-md:flex-col justify-center items-center mb-20'>
          Percentage per country
        </h3>
        <div className='mb-[20px] flex h-[400px] w-full justify-center'>
          <NivoChoropleth
            data={mapData}
            colors={['#caf0f8', '#90e0ef', '#00b4d8', '#0077b6', '#03045e']}
            valueFormat='.1%'
            domain={[0, 0.28]}
          />
        </div>
      </section>
      <section id='per-region' className='mt-10'>
        <h3 className='py-2 font-mono text-3xl max-md:text-2xl text-center flex max-md:flex-col justify-center items-center mb-20'>
          Average by region
        </h3>
        <div className='mb-[20px] flex h-[500px] w-full justify-center max-md:h-[500px] max-xl:h-[500px]'>
          <NivoBar
            data={avgRegion}
            keys={['Percentage']}
            indexBy='Region'
            colors={['#14213dbf']}
            borderWidth={2}
            borderColor={{
              from: 'color',
              modifiers: [['darker', 1.6]],
            }}
            labelTextColor='#fff'
            margin={{ left: 40, bottom: 140, top: 40, right: 10 }}
            mobileMargin={{ left: 40, bottom: 140, top: 30, right: 10 }}
            mobileLayout='vertical'
            xtickRotation={45}
            layout='vertical'
            yFormat='.1%'
          />
        </div>
      </section>
      <section id='per-country' className='mt-10'>
        <h3 className='py-2 font-mono text-3xl max-md:text-2xl text-center flex max-md:flex-col justify-center items-center mb-20'>
          Country detail
        </h3>
        <div className='mb-[20px] flex h-[2000px] w-full justify-center max-md:h-[2000px] max-xl:h-[2000px]'>
          <NivoBar
            xAxis
            data={mapData}
            keys={['value']}
            indexBy='label'
            colors={['#fca311bf']}
            borderWidth={2}
            borderColor={{
              from: 'color',
              modifiers: [['darker', 1.6]],
            }}
            labelTextColor='#fff'
            margin={{ left: 120, bottom: 40, top: 40, right: 10 }}
            mobileMargin={{ left: 80, bottom: 30, top: 30, right: 10 }}
            mobileLayout='horizontal'
            yFormat='.1%'
          />
        </div>
      </section>
      <p className='text-center text-xs'>
        All data are updated at the end of the 2023 and taken from{' '}
        <a
          className='text-[#fca311] font-bold'
          href='https://gender-data-hub-2-undesa.hub.arcgis.com/datasets/65ee4b9b43da442bb7e360ca83d4bb24/explore'
          rel='noopener noreferrer'
          target='_blank'
        >
          The gender data hub
        </a>
      </p>
    </>
  );
};

export default Page95;
