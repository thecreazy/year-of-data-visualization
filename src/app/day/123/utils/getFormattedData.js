...
import rawData from '../data/BAC.json';
...
export const employeesData = rawEmployees.map((el) => ({
  ...el,
  Employees: Intl.NumberFormat('en-EN').format(el.Employees),
}));

export const revenuesData = rawRevenuesData.map((el) => ({
  year: el.year,
  revenue: `${Intl.NumberFormat('en-EN').format(el.revenue)}$`,
  profit: el.profit ? `${Intl.NumberFormat('en-EN').format(el.profit)}$` : '-',
}));
